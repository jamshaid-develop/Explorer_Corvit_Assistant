...
<style>
    ...
    .main-header h1 { font-family: 'Space Grotesk', sans-serif; color: #e2e8f0; margin: 0; font-size: 1.4rem; }
...
with left_col:
    ...
    <div style="background:#0f172a; padding:1rem; border-radius:12px; border:1px solid #1e293b;">
        <h3 style="color:#e2e8f0; font-family:'Space Grotesk',sans-serif; margin:0; font-size:1.1rem;">🎓 Explorer Corvit</h3>
        <p style="color:#64748b; font-size:0.75rem; margin:4px 0 0 0;">AI Assistant — Corvit Systems Rawalpindi</p>
    </div>
    """, unsafe_allow_html=True)
    st.markdown("")
    ...
    for s in sessions[:6]:
        ...
        if st.button(f"{active}{sid} ({count})", key=f"sess_{sid}", use_container_width=True):
    ...
    if uploaded:
        for up_file in uploaded:
            ...
            st.success(f"✅ {up_file.name} — {n} chunks")

    stats = get_collection_stats()
    st.markdown(f"<p style='color:#475569; font-size:0.72rem; margin:0.3rem 0;'>🧠 {stats['total_chunks']} chunks in knowledge base</p>", unsafe_allow_html=True)
    st.markdown("<hr style='border-color:#1e293b; margin:0.8rem 0'>", unsafe_allow_html=True)
    ...
    st.markdown("<p style='color:#334155; font-size:0.68rem; margin-top:0.5rem;'>Powered by Groq<br>NAVTTC Certified</p>", unsafe_allow_html=True)
...
with main_col:
    ...
    if not st.session_state.messages:
        ...
        <div style="text-align:center; padding:2rem 1rem; color:#64748b;">
            ...
            <h3 style="color:#94a3b8; font-family:'Space Grotesk',sans-serif;">Welcome to Explorer Corvit Assistant!</h3>
    ...
    with col_input:
        user_input = st.text_input("message", placeholder="e.g. Python course ki fees kya hai?",
